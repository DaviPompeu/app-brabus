...
uses
  ...
  //UFormat é a unit que cria as mascaras no edits
...
begin
 ...
 if tipoCampo = 'Telefone' then          // Parametro que vai definir o que está sendo alterado, nesse caso são os dados do telefone
 begin
 fundo.Height := 265;                    //fundo do rec branco, redimensionar conforme a necessidade de edits
 EdtAlteraItem1.Height := 32;            // ajusta o edit1, pois a propriedade Height é alterada nas observações
// ComboGenero.Visible := false;
 ...
 lbl2 := lblEdicao2;                     // fechar a Aba de Edição, Puxar os dados que foram colocados nos edits
...
var
 UF : string;
begin
//Para essa formatação funcionar o estado no endereço deve está informado corretamente
...
begin
CadastroFisicoDadosBasico('Basicos', 'Dados básicos do cliente', Lbl_FisicoDadosSexo, Lbl_FisicoDadosDataNascimento, Lbl_FisicoDadosEmail );
end;
...
begin
CadastroJuridicoCnpjInscricao('CNPJ', 'CPNJ e Inscrição Estadual', Lbl_JuridicoCPNJCliente, Lbl_JuridicoInscricaoEstadual);
end;

procedure TFrmCadastroCliente.ListBoxEnderecoClick(Sender: TObject);
begin
CadastroJuridicoEndereco('Endereco', 'Endereço do cliente', lbl_JuridicoEnderecoRua, Lbl_JuridicoEnderecoBairro, Lbl_JuridicoEnderecoCep, Lbl_JuridicoEnderecoNumero, Lbl_JuridicoEnderecoEstado, Lbl_JuridicoEnderecoCidade);
end;
...
begin
CadastroJuridicoDadosBasicos('Dados Basicos', 'Dados Básicos', Lbl_JuridicoDadosBasicosEmail, Lbl_JuridicoDadosBasicosSite, Lbl_JuridicoDadosBasicosFax);
end;

procedure TFrmCadastroCliente.ListBoxItem4Click(Sender: TObject);
begin
CadastroJuridicoObeservacoes('Observacoes', 'Obeservações', Lbl_JuridicoObeservacaoCliente);
end;

procedure TFrmCadastroCliente.FisicoListEnderecoClick(Sender: TObject);
begin
CadastroFisicoEndereco('Endereço', 'Endereço do Cliente' ,Lbl_FisicoEnderecosRua, Lbl_FisicoEnderecosBairro, Lbl_FisicoEnderecosCep,  Lbl_FisicoEnderecosNumero,   Lbl_FisicoEnderecosEstado, Lbl_FisicoEnderecosCidade);
end;
...
begin
//EdicaoDadosFisicos('Observacao', 'Observações do Cliente', Lbl_FIsicoObservacaoText, LblConn, LblConn, LblConn, LblConn, LblConn, LblConn, LblConn);
CadastroFisicoObservacao('Observacao', 'Observações do Cliente', Lbl_FIsicoObservacaoText);
end;
...
// ajustando tela de cadastro de cliente fisico e juridico
 ...
 LblTituloCadastro.Text := 'Clientes Físicos';
 ImgFisico.Opacity := 1;
...
//mudando a opacidade do juridico para o fisico e do fisico para o juridico
 ...
 LblTituloCadastro.Text := 'Clientes Jurídicos';
...
begin
    ...
    begin
         ...
         StringResult := FloatToStr(RealResult);
          ...
          begin
          //SALVANDO OS DADOS DOS CLIENTE FÍSICOS
            Active := false;
            SQL.Clear;
            // SE LBL_FisicoNomeLCiente.txt e Lbl_FisicoCPFCliente.text for <> de vazio, então execulta o procedimento de insert na tab_cliente
             ...
             if StringResult = '10001' then // Se StringResult = 10001 significa que devemos fazer um insert, pois não existe nenhum campo na TAB_CODIGO
             begin
             Sql.Clear;
             ...
             begin
             Sql.Clear;
             SQL.ADD('UPDATE TAB_CODIGO SET CODIGO_CLIENTE=:CODIGODOCLIENTE'); // Se StringResult <> 10001 significa que na TAB_CODIGO tem um codigo já cadastrado
             ParamByName('CODIGODOCLIENTE').Value := StringResult;
             ExecSQL;
             end;
...
begin
// verificando qual o Tlayout está ativo para fazer o fechamento correto
...
begin
// fazendo o fechamento pelo botão voltar da tela de cadastro
  if LytCadFisicoNome.Visible = true then
    begin
...
begin
 ...
 LytCadFisicoNome.Visible := false;
 LytCadFisicoCpf.Visible := false;
 LytCadFisicoEndereco.Visible := true;     //cadastro do endereço do cliente
 LytCadFisicoDadosBasicos.Visible := false;
 LytCadFisicoObeservacao.Visible := false;
...
begin
 // FECHA CADASTRO DO ENDEREÇO DO CLIENTE
...
procedure TFrmCadastroCliente.CadastroFisicoObservacao(tipoCampo, titulo: string;
 lblEdicao1 :TLabel);
begin
 // ABRE CADASTRO DAS OBSERVAÇÕES DO CLIENTE
 LytCadastroFisico.Visible := false;
 LytCadFisicoTelefone.Visible := false;
 LytCadFisicoEndereco.Visible := false;
 LytCadFisicoNome.Visible := false;
...
begin
 // FECHA CADASTRO DAS OBSERVAÇÕES DO CLIENTE
...
procedure TFrmCadastroCliente.CadastroJuridicoEndereco(tipoCampo, titulo: string;
 lblEdicao1,  lblEdicao2,  lblEdicao3,  lblEdicao4,  lblEdicao5, lblEdicao6 :TLabel);
begin
 //CADASTRO DO ENDEREÇO DO CLIENTE JURIDICO
 LytCadastroFisico.Visible := false;
 LytCadastroJuridico.Visible := false;
 LytCadJuridicoNome.Visible := false;
 LytCadJuridicoInscricao.Visible := false;
...
begin
 // FECHA CADASTRO DO ENDEREÇO DO CLIENTE JURIDICO
...
procedure TFrmCadastroCliente.CadastroJuridicoCnpjInscricao(tipoCampo, titulo: string;
 lblEdicao1, lblEdicao2 :TLabel);
begin
 // CADASTRO DO CNPJ E INSCRIÇÃO ESTADUAL DO CLIENTE JURIDICO
 LytCadastroFisico.Visible := false;
 LytCadastroJuridico.Visible := false;
 LytCadJuridicoNome.Visible := false;
...
begin
 // FECHA CADASTRO DO CNPJ E INSCRIÇÃO ESTADUAL DO CLIENTE JURIDICO
...
procedure TFrmCadastroCliente.CadastroJuridicoObeservacoes(tipoCampo, titulo: string;
 lblEdicao1 :TLabel);
begin
 // CADASTRO DA OBSERVAÇÃO DO CLIENTE JURIDICO
 LytCadastroFisico.Visible := false;
 LytCadastroJuridico.Visible := false;
 LytCadJuridicoNome.Visible := false;
 LytCadJuridicoInscricao.Visible := false;
...
begin
 // FECHA CADASTRO DAS OBSERVAÇÕES DO CLIENTE JURIDICO
...
begin
 if LblTituloCadastro.Text = 'Clientes Físicos' then
 begin
 LytMessagemInfor.Visible := true;
 LblMessagemInfor.Text := 'Aqui você pode fazer o cadastro de clientes Físicos!';
 end
 else
 begin
 LytMessagemInfor.Visible := true;
 LblMessagemInfor.Text := 'Aqui você pode fazer o cadastro de clientes Jurídicos!';
 end;
